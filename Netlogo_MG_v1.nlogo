globals [ratio low-ratio mid-ratio per-shops per-bf per-sf profit-battery investment-month consumers non-consumers profit]
...
households-own [caste children? agentset-connected employement income-first role energy cheating-counter]
energy-provider-own [energy]
...
to sell-energy
  ...
  ask households [
    set consumers households with [income-first = "y" OR children? = "y"]

    ; setting up consumers when the neighbors have got energy
      if consumers = 0 [
        ask agentset-connected [
          let close-neighbors other min-n-of 5 households [distance myself]
          set consumers close-neighbors with [energy >= 1]]]

    if cheating-counter > 3 [set income-first "n"]

    set non-consumers households with [income-first = "n"]

    ask non-consumers [set size 1 set role 0] ]

  ask energy-provider [
    create-links-with consumers

    ; decresing energy of the producer and making profit
    ifelse (energy - (count consumers * demand-rise)) <= 0 [
      set energy (energy - (count consumers * demand-rise))] [
      set energy 0]
    set color (39 - energy)
    set profit (profit-battery * count consumers)]
...
to upgrade-income
  ask households [
    if energy > 12 [
      set demand-rise (demand-rise + 1)]]
end

to check-margin
  if count non-consumers != 0 [
    if (count consumers / count non-consumers) < 1 [
      ask n-of (0.2 * count non-consumers) non-consumers [
        set income-first "y"]]]
end

to replace-components
  let replacement-cost-one 3
  ask energy-provider [
    if ticks mod 6 = 0 [
      let replacement-cost (replacement-cost-one * count consumers)
      set profit (profit - replacement-cost)]]

  ; cheating happens leading to cutting off the services to some
  if ticks mod 5 = 0 [
    ask n-of (0.5 * count consumers) consumers [
      set income-first "n"
      set cheating-counter (cheating-counter + 1)]]
end
@#$#@#$#@
GRAPHICS-WINDOW
410
