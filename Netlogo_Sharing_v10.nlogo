globals [ratio low-ratio mid-ratio per-shops per-bf per-sf profit-battery investment-month maximum-batteries-sold producers non-producers consumers non-consumers]
...
to go
  ; delays are caused due to tests and registrations
  ;if (ticks mod 6) = 0[
    appoint-producer
    sell-batteries
    ;replace-components]
...
to appoint-producer
  ...
  ;subsidy-effect
  if subsidy-rooftop [
    ask n-of (0.5 * population) households [
      set income-first "y" ]]
  ...
  ask households [
    ...
    ask non-producers [set pcolor black]]
...
to sell-batteries
  ask producers [
    ; setting up consumers when the households have children and no patriarchy
    set agentset-consumer (agentset-connected with [children? = "y" AND role = 0])
    ...
    ; decreasing energy of the producer and making profit
    ...
    if count agentset-consumer > maximum-batteries-sold [
      ask n-of (count agentset-consumer - maximum-batteries-sold) agentset-consumer [set role 0]
      set agentset-consumer agentset-consumer with [role != 0]]
...
;      set my-producer producer-who]]
    ...
    set consumers (count households with [role = "consumer"]) ]
...
to upgrade-connectivity
  ask households with [role = 0 AND income-first = "y"] [
    if (one-of agentset-connected with [role = "consumer"]) != NOBODY [
      let me self
      ask one-of agentset-connected with [role = "consumer"][
        ask my-producer [
          if agentset-consumer != 0 [
