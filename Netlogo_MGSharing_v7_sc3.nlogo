...
energy-provider-own [energy  agentset-consumer]
grid-own [energy  agentset-consumer]
...
to setup-houses
  ...
  ask energy-provider [
    ...
    set energy 20
    set color (39 - energy)
    set agentset-consumer (one-of households in-radius 5 with [children? = "y"])]

  create-grid 1
  ask grid [
    set shape "box"
    set energy 2000
    set xcor 1 set ycor 1
    set color yellow]
end
...
to appoint-producer
  ...
  ask grid [
    if ticks < 101[
      if non-producers != NOBODY[
        set agentset-consumer (n-of (0.5 * count non-producers) non-producers)
        if ticks = 100 [
...
to upgrade-connectivity
  ask households with [role = 0] [
    if (one-of agentset-connected with [role = "consumer"]) != NOBODY [
      let me self
      if my-producer != 0[
        ask a-household [who] of my-producer [
        if agentset-consumer != 0 [
            set agentset-consumer (turtle-set agentset-consumer me)]]]]]
end
